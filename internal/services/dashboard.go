...
func CreateDashboard(apiKey, apiUrl, jsonFile string) (*models.Dashboard, error) {
...
func GetDashboardInfo(apiKey, apiUrl, uid string) (*models.DashboardGet, error) {
	client := http.Client{}
	...
	var dashboard models.DashboardGet
	err = json.Unmarshal(body, &dashboard)
	if err != nil {
		return nil, fmt.Errorf("error parsing response: %s", err)
	}
